    ...
    <div class="bg-white rounded-2xl shadow-lg p-10 w-full max-w-md" data-aos="fade-up">
      ...
      <form @submit.prevent="register">
        <div class="mb-6">
          ...
          <input v-model="name" type="text" id="name" class="w-full border rounded px-3 py-2" :class="{'border-red-500': errors.name}" required />
          <p v-if="errors.name" class="text-red-500 text-xs mt-1">{{ errors.name[0] }}</p>
        </div>
        <div class="mb-6">
          <label for="email" class="block mb-2 font-medium">Email</label>
          <input v-model="email" type="email" id="email" class="w-full border rounded px-3 py-2" :class="{'border-red-500': errors.email}" required />
        ...
        <button 
          class="bg-green-600 text-white px-6 py-3 rounded-full font-semibold text-lg w-full transition-opacity duration-300"
